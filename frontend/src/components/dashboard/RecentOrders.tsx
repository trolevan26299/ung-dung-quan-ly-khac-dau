                        ...
                        {recentOrders.length > 0 ? recentOrders.map((order: any) => (
                            <div key={order._id} className="flex items-center justify-between rounded-xl border border-gray-100 bg-gray-50/60 p-3.5 transition-colors hover:bg-gray-100">
                                ...
                                <div className="ml-3 shrink-0 text-right">
                                    <p className="font-semibold tabular-nums text-gray-900">{formatCurrency(order.totalAmount)}</p>
                                    <span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium ${getStatusColor(order.paymentStatus)}`}>
